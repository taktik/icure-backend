...
`;
  ...
  _filterChanged(e) {
      ...
      this.dataSource && this.dataSource.filter(text).then(items => this.value ? this.dataSource.get(this.value).then(res => res && !items.some(it=>it.id === res.id) ? [res].concat(items) : items) : items).then(items => this.set('items', items));
	}
