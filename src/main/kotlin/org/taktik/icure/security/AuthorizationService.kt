...
class AuthorizationService(val sessionLogic: AsyncSessionLogic, val healthcarePartyDAO: HealthcarePartyDAO) {
    ...
    suspend fun isPatient() = sessionLogic.getCurrentSessionContext().getUserDetails().authorities.contains(Roles.GrantedAuthority.ROLE_PATIENT)
    ...
    suspend fun isHcp() = sessionLogic.getCurrentSessionContext().getUserDetails().authorities.contains(Roles.GrantedAuthority.ROLE_HCP)
    ...
    suspend fun isUser() = sessionLogic.getCurrentSessionContext().getUserDetails().authorities.contains(Roles.GrantedAuthority.ROLE_USER)
