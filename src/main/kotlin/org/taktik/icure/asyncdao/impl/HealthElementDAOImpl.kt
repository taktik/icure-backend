...
internal class HealthElementDAOImpl(couchDbProperties: CouchDbProperties,
    ...
    override fun findByHCPartyAndCodes(healthCarePartyId: String, codeType: String, codeNumber: String): Flow<String> {
        ...
        return client.queryView<Array<String>, String>(createQuery("by_hcparty_and_codes").key(ComplexKey.of(healthCarePartyId, "$codeType:$codeNumber")).includeDocs(false)).mapNotNull { it.value }
    }
    ...
    override fun findByHCPartyAndTags(healthCarePartyId: String, tagType: String, tagCode: String) = flow<String> {
        val client = couchDbDispatcher.getClient(dbInstanceUrl)
        emitAll(client.queryView<Array<String>, String>(createQuery("by_hcparty_and_tags").key(ComplexKey.of(healthCarePartyId, "$tagType:$tagCode")).includeDocs(false)).mapNotNull { it.value })
   }
    ...
    override fun findByHCPartyAndStatus(healthCarePartyId: String, status: Int?): Flow<String> {
        ...
        return client.queryView<Array<String>, String>(createQuery("by_hcparty_and_status").key(ComplexKey.of(healthCarePartyId, status)).includeDocs(false)).mapNotNull { it.value }
    }
    ...
    override fun findByHCPartySecretPatientKeys(hcPartyId: String, secretPatientKeys: List<String>): Flow<HealthElement> {
        ...
        val result = client.queryViewIncludeDocs<Array<String>, String, HealthElement>(createQuery("by_hcparty_patient").keys(keys).includeDocs(true)).map { it.doc }
