...
class PatientDAOImpl(
...
) : GenericIcureDAOImpl<Patient>(Patient::class.java, couchDbProperties, couchDbDispatcher, idGenerator), PatientDAO {
	...
	override fun listPatientIdsByHcPartyGenderEducationProfession(healthcarePartyId: String, gender: Gender?, education: String?, profession: String?): Flow<String> = flow {
		...
		val viewQuery = createQuery(client, "by_hcparty_gender_education_profession")
			...
			.endKey(
				ComplexKey.of(
					healthcarePartyId, if (gender == null) ComplexKey.emptyObject() else gender.name,
					education
