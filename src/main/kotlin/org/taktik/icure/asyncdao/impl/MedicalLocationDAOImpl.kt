...
class MedicalLocationDAOImpl(couchDbProperties: CouchDbProperties,
    ...
    override fun byPostCode(postCode: String): Flow<MedicalLocation> = flow {
        val client = couchDbDispatcher.getClient(dbInstanceUrl)
        emitAll(client.queryViewIncludeDocs<String, String, MedicalLocation>(createQuery(client, "by_post_code").includeDocs(true).key(postCode)).map { it.doc })
    }
}
