...
class PatientLogicImpl(
...
) : GenericLogicImpl<Patient, PatientDAO>(sessionLogic), PatientLogic {
	...
	override suspend fun mergePatient(patient: Patient, fromPatients: List<Patient>): Patient? {
		...
		return fromPatients.fold(patient to listOf<Patient>()) { (p, others), o ->
			p.merge(o) to others + (
				modifyPatient(
					p.copy(
				...
				) ?: error("Cannot modify patient ${p.id}")
				)
		}.let { (p, others) ->
	...
	override fun solveConflicts(): Flow<Patient> =
		patientDAO.listConflicts().mapNotNull {
			patientDAO.get(it.id, Option.CONFLICTS)?.let { patient ->
