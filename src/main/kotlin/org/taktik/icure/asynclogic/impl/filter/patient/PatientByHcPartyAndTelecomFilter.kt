...
class PatientByHcPartyAndTelecomFilter(
	...
	override fun resolve(filter: PatientByHcPartyAndTelecomFilter, context: Filters) = flow {
		try {
			emitAll(patientLogic.listPatientIdsByHcPartyAndTelecomOnly(filter.searchString, filter.healthcarePartyId ?: getLoggedHealthCarePartyId(sessionLogic)))
