...
import org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20131001.be.fgov.ehealth.standards.kmehr.cd.v1.CDINCAPACITYREASON
...
class SoftwareMedicalFileImport(val patientLogic: PatientLogic,
    ...
    suspend fun importSMF(inputData: Flow<ByteBuffer>,
        ...
        kmehrMessage.folders.forEach { folder ->
            ...
            createOrProcessPatient(folder.patient, author, res, saveToDatabase, dest, folder.transactions?.flatMap { it.findItems { it.cds.find { it.s == CDITEMschemes.CD_ITEM }?.value == "insurancystatus" } }?.firstOrNull()?.let {
    ...
    private suspend fun parseGenericTransaction(trn: TransactionType,
        ...
        val (services, subContacts) = trnItems.filter { item ->
            ...
            val (cdItem, label) = mapItem(item, mappings, language)
    ...
    private fun mapItem(item: ItemType, mappings: Map<String, List<ImportMapping>>, language: String): Pair<String, String> {
        ...
        val mapping = mappings[guessedCdItem]?.find { (it.lifecycle == "*" || it.lifecycle == item.lifecycle?.cd?.value?.value()) && ((it.content == "*") || item.hasContentOfType(it.content)) }
        val cdItem = mapping?.cdItem ?: guessedCdItem
        val label = if (cdItem == "parameter") "Observation" else
        ...
        val mapserv = mapOf(
                "incapacité de" to
                        item.contents.find { it.incapacity != null }?.let {
                            ...
                            Pair(
                                    Content(stringValue = it),
                                    listOf(CodeStub.from("CD-INCAPACITY", it, "1"))
                            )
                        },
                ...
                "pour cause de" to
                        item.contents.find { it.incapacity != null }?.let {
                            ...
                            Pair(
                                    Content(stringValue = it),
                                    listOf(CodeStub.from("CD-INCAPACITYREASON", it, "1"))
                            )
        ...
        return HealthElement(
                ...
                plansOfAction = kmehrIndex.approachFor.filter { (_,v) -> v.contains(mfId) }.mapNotNull { (k,_) ->
                    kmehrIndex.itemIds[k]?.second?.let { item ->
                        val (_, label) = mapItem(item, mappings, language)
