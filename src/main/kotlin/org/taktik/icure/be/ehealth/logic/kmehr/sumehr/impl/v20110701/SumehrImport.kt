...
import org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20110701.be.fgov.ehealth.standards.kmehr.cd.v1.CDTRANSACTIONschemes
...
class SumehrImport(val patientLogic: PatientLogic,
    ...
    private suspend fun parseGenericTransaction(trn: TransactionType,
        ...
        val (hes, svcs, sctcs) = items.fold(Triple(listOf<HealthElement>(), listOf<Service>(), listOf<SubContact>())) { (hes, svcs, sctcs), item ->
            val cdItem = item.cds.find { it.s == org.taktik.icure.services.external.rest.v1.dto.be.ehealth.kmehr.v20161201.be.fgov.ehealth.standards.kmehr.cd.v1.CDITEMschemes.CD_ITEM }?.value?.let {if (it == "problem") "healthcareelement" else it} ?: "note"
            val mapping =
            ...
            when {
                (listOf("healthcareelement", "allergy", "adr", "risk", "socialrisk").contains(cdItem)) -> {
                    val he = parseHealthcareElement(mapping?.cdItem
                ...
                Content(medicationValue = Medication(
                        substanceProduct = item.contents.filter { it.substanceproduct != null }.firstOrNull()?.let {
                            it.substanceproduct?.let {
                                Substanceproduct(intendedcds = it.intendedcd?.let { listOf(CodeStub.from(it.s, it.value, it.sv)) }
                                        ?: listOf(),
                        ...
                        medicinalProduct = item.contents.filter { it.medicinalproduct != null }.firstOrNull()?.let {
                            it.medicinalproduct?.let {
                                Medicinalproduct(intendedcds = it.intendedcd?.let { listOf(CodeStub.from(it.s.toString(), it.value, it.sv)) }
                                        ?: listOf(),
                                        intendedname = it.intendedname.toString())
                            }
                        },
