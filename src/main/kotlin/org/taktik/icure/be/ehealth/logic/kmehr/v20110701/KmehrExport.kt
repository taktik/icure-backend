	...
	open fun createItemWithContent(he: HealthElement, idx: Int, cdItem: String, contents: List<ContentType>): ItemType? {
		return ItemType().apply {
			...
			lifecycle = LifecycleType().apply {
				cd = CDLIFECYCLE().apply {
					s = "CD-LIFECYCLE"
					value = if ((he.status and 2) != 0 || (he.closingDate ?: 0 > FuzzyValues.getCurrentFuzzyDate()))
						CDLIFECYCLEvalues.INACTIVE
					else
		...
		return FolderType().apply {
			...
			transactions.add(
				TransactionType().apply {
					...
					ids.add(IDKMEHR().apply { s = IDKMEHRschemes.LOCAL; sl = "iCure-Item"; sv = ICUREVERSION; value = ssc.id ?: patient.id })
