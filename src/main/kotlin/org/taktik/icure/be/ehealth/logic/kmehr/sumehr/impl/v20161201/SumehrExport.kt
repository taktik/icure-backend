...
class SumehrExport(
	...
	internal suspend fun fillPatientFolder(folder: FolderType, p: Patient, sfks: List<String>, sender: HealthcareParty, language: String, config: Config, comment: String?, excludedIds: List<String>, includeIrrelevantInformation: Boolean, decryptor: AsyncDecrypt?, services: List<Service>?, healthElements: List<HealthElement>?): FolderType {
		...
		addActiveServicesAsCD(
			hcpartyIds, sfks, trn, "patientwill", CDCONTENTschemes.CD_PATIENTWILL,
			listOf(
			...
			excludedIds, includeIrrelevantInformation, decryptor, services, language
		)

		addActiveServicesAsCDPatientWillChoice(
			hcpartyIds, sfks, trn, "patientwill", CDCONTENTschemes.CD_PATIENTWILL_HOS,
			listOf(
				"hospitalisation"
			),
			excludedIds, includeIrrelevantInformation, decryptor, services, language
		...
		addActiveServicesAsCDPatientWillChoice(
			hcpartyIds, sfks, trn, "patientwill", CDCONTENTschemes.CD_PATIENTWILL_RES,
			listOf(
				"resuscitation"
			),
			excludedIds, includeIrrelevantInformation, decryptor, services, language
	...
	internal suspend fun createVaccineItem(svc: Service, itemIndex: Int, language: String): ItemType? {
		val item = createItemWithContent(
			svc, itemIndex, "vaccine",
			listOf(
	...
	override fun createItemWithContent(svc: Service, idx: Int, cdItem: String, contents: List<ContentType>, localIdName: String, language: String, texts: List<TextType>?, link: LnkType?, config: Config, altBeginMoment: Long?, altEndMoment: Long?): ItemType? {
		if (ServiceStatus.isAbsent(svc.status) || svc.tags.any { t -> t.type == "CD-LIFECYCLE" && t.code == "notpresent" }) {
			return null
		}
		return super.createItemWithContent(svc, idx, cdItem, contents, localIdName, language, texts, link, config, altBeginMoment, altEndMoment)
	}
	...
	internal suspend fun addHealthCareElements(
