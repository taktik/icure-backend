    ...
    private fun getProtocolCode(lines: List<String>, i: Int, isStandardFormat: Boolean, demandDate: Date?): String? {
        ...
        if (isStandardFormat) {
            try {
                ...
                code = computeProtocolCode(lines[i + 1].substring(0, Math.min(24, lines[i + 1].length)).trim { it <= ' ' },
            ...
            try {
                code = computeProtocolCode(lines[i + 1].substring(0, 24).trim { it <= ' ' },
...
//come before the birth date.
    ...
    override fun doExport(sender: HealthcareParty?, recipient: HealthcareParty?, patient: Patient?, date: LocalDateTime?, ref: String?, text: String?) : Flow<DataBuffer> {
        ...
        if (sender!!.nihii != null && recipient!!.nihii != null) { //1
            pw.print(sender.nihii!!.replace("([0-9])([0-9]{5})([0-9][0-9])([0-9][0-9][0-9])".toRegex(),
                    "$1/$2/$3/$4").replace("[\\r\\n]".toRegex(), "") + "\r\n")
            //2
            ...
            pw.print((StringUtils.rightPad(StringUtils.substring(senderPhone.map { obj: Telecom -> obj.telecomNumber }.orElse(""), 0, 25), 25) +
                    StringUtils.rightPad(StringUtils.substring(senderPhone.map { obj: Telecom -> obj.telecomNumber }.orElse(""), 0, 25), 25)).replace("[\\r\\n]".toRegex(), "") + "\r\n")
            //6
            pw.print("\r\n")
            //7
            pw.print(df.format(Date()) + "\r\n")
            //8
            pw.print(recipient.nihii!!.replace("([0-9])([0-9]{5})([0-9][0-9])([0-9][0-9][0-9])".toRegex(),
                    "$1/$2/$3/$4").replace("[\\r\\n]".toRegex(), "") + "\r\n")
            //9
            pw.print((StringUtils.rightPad(StringUtils.substring(recipient.lastName, 0, 24), 24) +
                    StringUtils.rightPad(StringUtils.substring(if (recipient.firstName == null) "" else recipient.firstName, 0, 16), 16)).replace("[\\r\\n]".toRegex(), "") + "\r\n")
            pw.print("#A" + (if (Strings.isNullOrEmpty(patient.ssin)) "" else patient.ssin) + "\r\n")
            //2
            pw.print((StringUtils.rightPad(StringUtils.substring(patient.lastName, 0, 24), 24) +
                    StringUtils.rightPad(StringUtils.substring(if (patient.firstName == null) "" else patient.firstName, 0, 16), 16)).replace("[\\r\\n]".toRegex(), "") + "\r\n")
            //3
            ...
            pw.print((StringUtils.rightPad(StringUtils.substring(patientAddress.map { obj: Address -> obj.street }.orElse(""), 0, 24), 24) +
                    StringUtils.rightPad(StringUtils.substring(patientAddress.map { obj: Address -> obj.houseNumber }.orElse(""), 0, 7), 7)).replace("[\\r\\n]".toRegex(), "") + "\r\n")
    ...
    private fun computeProtocolCode(name: String, first: String, birth: Long, req: Long, code: String): String {
        return ("" + StringUtils.substring(name.replace(" ".toRegex(), ""), 0, 16)
