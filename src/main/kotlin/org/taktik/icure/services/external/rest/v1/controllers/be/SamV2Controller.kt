...
import org.taktik.icure.services.external.rest.v1.dto.samv2.*
import org.taktik.icure.services.external.rest.v1.mapper.samv2.*
...
class SamV2Controller(
	...
	fun findPaginatedVmpsByVmpCode(
		...
		@Parameter(description = "Number of rows", required = false) @RequestParam("limit") limit: Int?
	) = mono {
		val realLimit = limit ?: DEFAULT_LIMIT
		val paginationOffset = PaginationOffset(startKey, startDocumentId, null, realLimit + 1)
	...
	private suspend fun addProductIdsToAmps(amps: Collection<AmpDto>): List<AmpDto> {
		...
		return amps.map {
			if (it.ampps.any { it.dmpps.isNotEmpty() }) it.copy(
				ampps = it.ampps.map {
					it.copy(
