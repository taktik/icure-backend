	...
	if (doc.java_type === 'org.taktik.icure.entities.Contact' && !doc.deleted) {
		if (doc.delegations) {
			...
			doc.services.forEach(function (s) {
				if (s.label) {
					var r = s.label.toLowerCase().replace(/[^A-Za-z0-9]/g, function (a) {
						return latin_map[a] || ""
					});
					(ss[r] || (ss[r] = [])).push(s._id);
				}
			})
			Object.keys(ss).forEach(function (sLabel) {
				Object.keys(doc.delegations).forEach(function (k) {
					doc.secretForeignKeys.forEach(function (fk) {
						ss[sLabel].forEach(function(sId) {
							emit([k, fk, sLabel], sId)
						})
					})
				})
			})
