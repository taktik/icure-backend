map = function (doc) {
  var emit_normalized_substrings = function (k, txt, docId, latin_map) {
    var r = txt.toLowerCase().replace(/[^A-Za-z0-9]/g, function (a) {
      return latin_map[a] || ""
    });
  ...
  if (doc.java_type == 'org.taktik.icure.entities.Patient' && !doc.deleted) {
    ...
    if (doc.lastName || doc.firstName) {
      if (doc.patientHealthCareParties) {
        doc.patientHealthCareParties.forEach(function (phcp) {
          emit_normalized_substrings(phcp.healthcarePartyId, (doc.lastName ? doc.lastName : "") + (doc.firstName ? doc.firstName : ""), doc._id, latin_map);
        });
      }
    }
    if (doc.maidenName && doc.maidenName != doc.lastName) {
      if (doc.patientHealthCareParties) {
        doc.patientHealthCareParties.forEach(function (phcp) {
          emit_normalized_substrings(phcp.healthcarePartyId, doc.maidenName, doc._id, latin_map);
        });
      }
    }
    if (doc.spouseName && doc.spouseName != doc.lastName) {
      if (doc.patientHealthCareParties) {
        doc.patientHealthCareParties.forEach(function (phcp) {
          emit_normalized_substrings(phcp.healthcarePartyId, doc.spouseName, doc._id, latin_map);
        });
      }
    }
  }
