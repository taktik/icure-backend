map = function(doc) {
    var emit_normalized_substrings = function(doc, txt, latin_map) {
        var r = txt ? txt.toLowerCase().replace(/[^A-Za-z0-9]/g,function(a){return latin_map[a]||""}) : null;
        ...
        if (r) {
            for (var i = 0; i <= r.length - 3; i++) {
                emit([ doc.entityType, r.substr(i, r.length - i)], summary);
            }
        } else {
            emit([doc.entityType, null], summary);
        }
    };


    if (doc.java_type == 'org.taktik.icure.entities.EntityTemplate' && !doc.deleted) {
        ...
        "ₒ":"o","ᵣ":"r","ᵤ":"u","ᵥ":"v","ₓ":"x"};
        emit_normalized_substrings(doc, doc.descr, latin_map);
    }
};
