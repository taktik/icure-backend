map = function(doc) {
	var compute_normalized_substrings = function(acc,txt,latin_map) {
		var r = txt.toLowerCase().replace(/[^A-Za-z0-9]/g,function(a){return latin_map[a]||""});
	...
	if (doc.java_type == 'org.taktik.icure.entities.Insurance' && !doc.deleted) {
		...
		if (doc.name) {
			...
			Object.keys(doc.name).forEach(function (l) {
				compute_normalized_substrings(acc, doc.name[l], latin_map);
			});
