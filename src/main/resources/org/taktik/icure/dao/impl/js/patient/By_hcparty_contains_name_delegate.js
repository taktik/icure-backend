map = function(doc) {
    var emit_normalized_substrings = function (k, txt, doc, latin_map) {
        var r = txt.toLowerCase().replace(/[^A-Za-z0-9]/g, function (a) {
            return latin_map[a] || ""
        });
        for (var i = 0; i <= r.length - 3; i++) {
            emit([k, (r.substr(i, r.length - i))], doc._id);
        }
    };
    ...
    if (doc.java_type == 'org.taktik.icure.entities.Patient' && !doc.deleted) {
        ...
        if (doc.maidenName && doc.maidenName != doc.lastName) {
            if (doc.delegations) {
                Object.keys(doc.delegations).forEach(function (k) {
                    emit_normalized_substrings(k, doc.maidenName, doc, latin_map);
                });
            }
        }
        if (doc.spouseName && doc.spouseName != doc.lastName) {
            if (doc.delegations) {
                Object.keys(doc.delegations).forEach(function (k) {
                    emit_normalized_substrings(k, doc.spouseName, doc, latin_map);
                });
            }
        }
    }
