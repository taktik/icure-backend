...
<%
    ...
    invoice.getInvoicingCodes().forEach(c -> {if (c.getCode() == null && c.getTarificationId() != null && c.getTarificationId().contains("|")) {c.setCode(c.getTarificationId().split("\\|")[1]);}});

	pageContext.setAttribute("hcp_name", hcp.getCompanyName() != null  ? hcp.getCompanyName() : (hcp.getFirstName() + " " + hcp.getLastName()) );
    ...
    pageContext.setAttribute("pat_address", homeAddress != null ? homeAddress : patient.getAddresses().stream().findFirst().orElse(new Address()));
...
<div class="invoice-box">
	<table cellpadding="0" cellspacing="0">
		<tr class="top">
			<td class="p50">
				Justificatif #: ${invoice.invoiceReference}<br>
				Date: <fmt:formatDate value="${invoice_date}" pattern="dd/MM/yyyy"/><br>
			</td>
			<td class="p50" style="text-align: center;">
				<svg id="barcode"></svg><br>
				<span>${invoice.thirdPartyReference}</span><br>
				<c:if test="${invoice.printedDate!=null}"><span style="color: red">DUPLICATA</span></c:if>
			</td>
		</tr>
				...
				<table>
					<tr>
						<td>
							${hcp_name}<br>
							${hcp.speciality}<br>
							${hcp_address.street} ${hcp_address.houseNumber} ${hcp_address.postboxNumber != null ? "b. ".concat(hcp_address.postboxNumber) : ""}
							${hcp_address.postalCode} ${hcp_address.city}<br>
							Tel: ${hcp_phone}<br>
							TVA/CBE: ${hcp.cbe}<br>
							IBAN: ${hcp.bankAccount}<br>
							${hcp_email}
						</td>
						<td>
							${patient.gender==null || patient.gender.name == "female" ? "Madame" : "Monsieur"} ${patient.firstName} ${fn:toUpperCase(patient.lastName)}<br>
							${pat_address.street} ${pat_address.houseNumber} ${pat_address.postboxNumber != null ? "b. ".concat(pat_address.postboxNumber) : ""}<br>
							${pat_address.postalCode} ${pat_address.city}<br>
							${pat_address.country}
						</td>
					</tr>
				</table>
	...
	<table>
		<tr class="heading">
			<td>Prestations remboursables</td>
			<td class="p12">Honoraire porté en compte</td>
			<td class="p12">Intervention mutuelle</td>
			<td class="p12">Honoraire convention</td>
			<td class="p12">∆ et Suppléments</td>
		</tr>
		<c:set var="totalACharge" value="${0.0}" />
		<c:set var="totalSuppl" value="${0.0}" />
		<c:set var="totalRemb" value="${0.0}" />
		<c:set var="totalConv" value="${0.0}" />
		<c:set var="totalDelta" value="${0.0}" />
		<c:forEach items="${invoice.invoicingCodes}" var="c">
			<c:set var="totalACharge" value="${totalACharge + (c.reimbursement!=null?c.reimbursement:0) + (c.patientIntervention!=null?c.patientIntervention:0)}" />
			<c:set var="totalSuppl" value="${totalSuppl + (c.doctorSupplement != null ? c.doctorSupplement:0)}" />
			<c:set var="totalRemb" value="${c.reimbursement != null && totalRemb != null ? totalRemb + c.reimbursement : null}" />
			<c:set var="totalConv" value="${c.conventionAmount != null && totalConv != null ? totalConv + c.conventionAmount : null}" />
			<c:set var="totalDelta" value="${c.conventionAmount != null && totalDelta != null ? (totalDelta + (c.reimbursement != null ? c.reimbursement : 0) + (c.patientIntervention != null ? c.patientIntervention : 0) + (c.doctorSupplement != null ? c.doctorSupplement:0) - c.conventionAmount) : null}" />
			<tr class="item">
				<td> ${c.units!=null && c.units>1 ? c.units:""} ${c.code}: ${c.label}</td>
				<td class="right"><fmt:formatNumber value="${0.001+(c.reimbursement!=null?c.reimbursement:0) + (c.patientIntervention!=null?c.patientIntervention:0)}" pattern="0.00 €"/></td>
				<td class="right"><fmt:formatNumber value="${0.001+(c.reimbursement!=null?c.reimbursement:0)}" pattern="0.00 €"/></td>
				<td class="right">
					<c:if test="${c.conventionAmount!=null}"><fmt:formatNumber value="${c.conventionAmount}" pattern="0.00 €"/></c:if>
					<c:if test="${c.conventionAmount==null}">nc</c:if>
				</td>
				<td class="right">
					<c:if test="${c.conventionAmount!=null}"><fmt:formatNumber value="${0.001+(c.reimbursement!=null?c.reimbursement:0) + (c.patientIntervention!=null?c.patientIntervention:0) + (c.doctorSupplement!=null?c.doctorSupplement:0) - (c.conventionAmount!=null?c.conventionAmount:0)}" pattern="0.00 €"/></c:if>
					<c:if test="${c.conventionAmount==null}">na</c:if>
				</td>
			</tr>
		</c:forEach>
		<tr class="item bold">
			<td>Sous-totaux :</td>
			<td class="right"><fmt:formatNumber value="${0.001+totalACharge}" pattern="0.00 €"/></td>
			<td class="right"><fmt:formatNumber value="${0.001+totalRemb}" pattern="0.00 €"/></td>
			<td class="right">
				<c:if test="${totalConv!=null}"><fmt:formatNumber value="${0.001+totalConv}" pattern="0.00 €"/></c:if>
				<c:if test="${totalConv==null}">na</c:if>
			</td>
			<td class="right">
				<c:if test="${totalDelta!=null}"><fmt:formatNumber value="${0.001+totalDelta}" pattern="0.00 €"/></c:if>
				<c:if test="${totalDelta==null}">na</c:if>
			</td>
		</tr>
		<tr class="item last">
			<td>Totaux Suppléments / Totaux ∆ / Totaux ∆ et suppl.</td>
			<td class="right"><fmt:formatNumber value="${0.001+totalSuppl}" pattern="0.00 €"/></td>
			<td></td>
			<td class="right">
				<c:if test="${totalConv!=null}"><fmt:formatNumber value="${0.001+totalACharge - totalConv}" pattern="0.00 €"/></c:if>
				<c:if test="${totalConv==null}">na</c:if>
			</td>
			<td class="right">
				<c:if test="${totalDelta!=null}"><fmt:formatNumber value="${0.001+totalDelta}" pattern="0.00 €"/></c:if>
		...
		<tr class="total">
			<td>Total porté en compte</td>
			<td class="right"><fmt:formatNumber value="${0.001+totalACharge + totalSuppl}" pattern="0.00 €"/></td>
			<td class="right"></td>
			<td class="right"></td>
			<td class="right"></td>
		</tr>
