...
public class PatientByHcPartyDateOfBirthBetweenFilter implements Filter<String, Patient, org.taktik.icure.dto.filter.patient.PatientByHcPartyDateOfBirthBetweenFilter> {
    ...
    @Override
    public Set<String> resolve(org.taktik.icure.dto.filter.patient.PatientByHcPartyDateOfBirthBetweenFilter filter, Filters context) {
        try {
            return new HashSet<>(patientLogic.listByHcPartyDateOfBirthIdsOnly(filter.getMinDateOfBirth(), filter.getMaxDateOfBirth(), filter.getHealthcarePartyId() != null ? filter.getHealthcarePartyId() : getLoggedHealthCarePartyId()));
