...
public class PatientByHcPartyAndExternalIdFilter implements Filter<String, Patient, org.taktik.icure.dto.filter.patient.PatientByHcPartyAndExternalIdFilter> {
    ...
    @Override
    public Set<String> resolve(org.taktik.icure.dto.filter.patient.PatientByHcPartyAndExternalIdFilter filter, Filters context) {
        try {
            return new HashSet<>(patientLogic.listByHcPartyAndExternalIdsOnly(filter.getExternalId(), filter.getHealthcarePartyId() != null ? filter.getHealthcarePartyId() : getLoggedHealthCarePartyId()));
