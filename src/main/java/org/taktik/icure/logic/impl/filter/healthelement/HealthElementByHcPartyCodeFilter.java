...
public class HealthElementByHcPartyCodeFilter implements Filter<String, HealthElement, org.taktik.icure.dto.filter.healthelement.HealthElementByHcPartyCodeFilter> {
	...
	@Override
	public Set<String> resolve(org.taktik.icure.dto.filter.healthelement.HealthElementByHcPartyCodeFilter filter, Filters context) {
		try {
			String hcPartyId = filter.getHealthCarePartyId() != null ? filter.getHealthCarePartyId() : getLoggedHealthCarePartyId();
			return new HashSet<>(healthElementLogic.findByHCPartyAndCodes(hcPartyId, filter.getCodeType(), filter.getCodeNumber()));
