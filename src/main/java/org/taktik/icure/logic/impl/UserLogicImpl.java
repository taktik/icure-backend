...
public class UserLogicImpl extends PrincipalLogicImpl<User> implements UserLogic {
	...
	public User setProperties(User user, List<Property> properties) {
		for (Property p : properties) {
			...
			if (!prop.isPresent()) {
				user.getProperties().add(new Property(new PropertyType(p.getTypedValue().getType(), p.getType().getIdentifier()), new TypedValue(p.getTypedValue().getType(), p.getValue())));
			} else {
