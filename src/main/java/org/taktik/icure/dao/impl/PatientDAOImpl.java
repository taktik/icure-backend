...
class PatientDAOImpl extends GenericIcureDAOImpl<Patient> implements PatientDAO {
	...
	public List<String> listIdsByHcPartyAndNameContainsFuzzy(String searchString, String healthcarePartyId, Integer limit) {
		...
		ViewQuery viewQuery = (limit == null ? createQuery("by_hcparty_contains_name") : createQuery("by_hcparty_contains_name").limit(limit)).startKey(ComplexKey.of(healthcarePartyId, name)).endKey(ComplexKey.of(healthcarePartyId, name == null ? ComplexKey.emptyObject() : name + "\ufff0")).includeDocs(false);
	...
	public List<String> listIdsOfHcPartyNameContainsFuzzy(String searchString, String healthcarePartyId, Integer limit) {
		String name = (searchString!=null)? StringUtils.sanitizeString(searchString):null;
		ViewQuery viewQuery = (limit == null ? createQuery("of_hcparty_contains_name") : createQuery("of_hcparty_contains_name").limit(limit)).startKey(ComplexKey.of(healthcarePartyId, name)).endKey(ComplexKey.of(healthcarePartyId, name == null ? ComplexKey.emptyObject() : name + "\ufff0")).includeDocs(false);
		return new ArrayList<>(new TreeSet<>(db.queryView(viewQuery, String.class)));
	}
