...
public class SimpleViewGenerator {
	...
	public Map<String, DesignDocument.View> generateViews(
		...
		eachMethod(repositoryClass, input -> {
            if (hasAnnotation(input, GenerateView.class)) {
                generateView(views, input, null);
            }
            return false;
        });
	...
	private void createDeclaredViews(
		...
		eachAnnotation(klass, Views.class, input -> {
	...
	private String resolveTypeDiscriminator(final Class<?> persistentType) {
		...
		eachField(persistentType, input -> {
		...
		eachMethod(persistentType, input -> {
            if (hasAnnotation(input, TypeDiscriminator.class)) {
                discrimintators
                        .add("doc."
	...
	private boolean hasTypeDiscriminatorFieldOrMethod(
		...
		Collection<?> hits = eachField(persistentType, input -> hasAnnotation(input, TypeDiscriminator.class));
		if (!hits.isEmpty()) {
			return true;
		}

		hits = eachMethod(persistentType, input -> hasAnnotation(input, TypeDiscriminator.class));
		return !hits.isEmpty();
	}
